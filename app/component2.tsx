...
const getListingTag = (project: Project) => {
...
const getListingPrice = (project: Project) => {
  if (project.type === 'rent') {
...
const getRentalHighlights = (project: Project) => {
  const highlights = [
    project.furnishing,
    project.bhk !== null ? `${project.bhk} BHK` : null,
    project.avialablefrom ? `Available from ${project.avialablefrom}` : null,
  ].filter(Boolean);

  return highlights.length > 0 ? highlights.join(' • ') : 'Contact for more details';
};
...
export default function Component2() {
  ...
  useEffect(() => {
    const fetcheddata = async () => {
      try {
        const db = await fetch(`${API_BASE_URL}/post/all`, {
          method: 'GET',
          headers: {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <div className="relative inline-block mb-6">
            <h1 className="text-3xl font-bold text-gray-900 pb-2">Properties Listed here</h1>
          ...
          {filteredProjects.map((property) => (
            ...
            <div key={property.postid} className="bg-white rounded-2xl p-3 border border-gray-200 shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col xl:flex-row gap-4">
              <div className="relative w-full xl:w-[35%] h-64 sm:h-72 rounded-xl overflow-hidden flex-shrink-0 group cursor-pointer">
                <img
                  src={property.thumnailimage || FALLBACK_IMAGE}
                ...
                <div className="absolute bottom-4 left-4 right-4 text-white">
                  <p className="text-2xl font-bold mb-0.5 text-blue-300">{getListingPrice(property)}</p>
                  
                  <p className="text-sm text-gray-200 mb-2 truncate">{getProjectLocation(property)}</p>
                  <p className="text-xs font-medium bg-white/20 inline-block px-2 py-1 rounded backdrop-blur-sm">{getProjectDetails(property)}</p>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-between py-2">
                <div className="flex gap-4 items-center bg-gray-50 rounded-xl p-3 border border-gray-100">
                  ...
                  <div>
                    <h3 className="text-xl font-bold mb-1 leading-tight">{property.title || 'Untitled Property'}</h3>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                  <div className="bg-amber-50/50 rounded-xl p-4 border border-amber-100">
                    ...
                    <p className="text-sm font-medium text-gray-800 mt-2 mb-3 leading-relaxed">{getRentalHighlights(property)}</p>
                  </div>
                  ...
                  <div className="bg-teal-50/50 rounded-xl p-4 border border-teal-100">
                    ...
                    <p className="text-sm font-medium text-gray-800 mt-2 mb-3">
                      Access to <span className="font-bold text-teal-700">{property.tagswork?.length || 0}+</span> Society Amenities
                    </p>
                    <span className="text-blue-700 text-sm font-semibold hover:text-blue-900 flex items-center gap-1">
                      View Now <span className="text-lg leading-none">›</span>
                    </span>
                  </div>
                </div>
              </div>
